...
describe('RedisService', () => {
  ...
  it('maps a null session transaction result to a safe platform error', async () => {
    const client = {
      defineCommand: vi.fn(),
    ...
    const service = new RedisService(client as never, config as never);

    await expect(
      service.createAuthenticationSession('user-1', 'session-1', 'hmac'),
    ).rejects.toEqual(new PlatformError('SERVICE_UNAVAILABLE'));
  });

  it('maps a session rotation failure to a safe platform error', async () => {
    const client = {
      defineCommand: vi.fn(),
      rotateAuthenticationSession: vi.fn().mockRejectedValue(new Error('down')),
      status: 'ready',
    };
    const service = new RedisService(client as never);

    await expect(
      service.rotateAuthenticationSession(
        'session-1',
        'presented',
        'replacement',
      ),
  ...
  it('maps a session revocation failure to a safe platform error', async () => {
    const client = {
      defineCommand: vi.fn(),
      revokeAuthenticationSession: vi.fn().mockRejectedValue(new Error('down')),
      status: 'ready',
    };
    const service = new RedisService(client as never);

    await expect(
      service.revokeAuthenticationSession('session-1', 'hmac'),
    ).rejects.toEqual(new PlatformError('SERVICE_UNAVAILABLE'));
  });

  it('maps a bulk session revocation failure to a safe platform error', async () => {
    const client = {
      defineCommand: vi.fn(),
      revokeUserAuthenticationSessions: vi
        .fn()
    ...
    await expect(
      service.revokeUserAuthenticationSessions('user-1'),
    ).rejects.toEqual(new PlatformError('SERVICE_UNAVAILABLE'));
  });
});
